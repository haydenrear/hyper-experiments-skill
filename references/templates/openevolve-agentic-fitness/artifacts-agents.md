# Agent instructions — {{experiment_id}} (openevolve-agentic-fitness variant)

Operating instructions for any LLM or agent working inside this experiment's
directory. This experiment uses the **openevolve-agentic-fitness** variant:
an OpenEvolve loop drives the search inside `code/`, with periodic
agentic fitness research reranking over retained programs. The
hyper-experiments protocol still
...
- `logs/fitness_events/` — JSONL evidence, program cards, score epochs,
  and score updates produced by the agentic fitness researcher.
