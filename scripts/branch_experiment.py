...
def main() -> int:
    ...
    if openevolve_db_report is not None:
        print(f"  0. ACP server (automatic for the default config.yaml):")
        print(f"     `uv run run-openevolve` starts a fresh server for this")
        print(f"     child experiment, waits for `/v1/models`, overrides")
        print(f"     the loaded OpenEvolve config to the auto-picked port,")
        print(f"     and stops the server when the run exits. JSONL traces go to")
        ...
        print(f"     Deploy/run output with `uv run run-best-program` from")
        print(f"     inside {exp_dir.relative_to(root)}/code/.")
